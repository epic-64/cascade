...
/** Configuration for game-specific reconnection behavior. Each game implements this trait to customize the reconnection
  * flow.
  */
...
/** Helper object for managing session reconnection flow. Provides standardized logic for checking and attempting
  * reconnection.
  */
object ReconnectionHelper:

  /** Check for existing session and attempt reconnection if found. Call this during game initialization.
    *
    * @param config
    *   Game-specific reconnection configuration
    * @return
    *   true if reconnection is being attempted, false if no session found
    */
  ...
  /** Attempt to reconnect to an existing game session.
    *
    * @param session
    *   The session data to reconnect with
    * @param config
    *   Game-specific reconnection configuration
    */
  ...
  /** Handle a successful rejoin response from the server. Updates session and notifies config.
    *
    * @param config
    *   Game-specific reconnection configuration
    * @param playerId
    *   The player ID confirmed by server
    * @param gameId
    *   The game ID confirmed by server
    * @param playerName
    *   The player name (from session or server)
    */
  ...
  /** Handle a failed rejoin response from the server. Clears session and notifies config.
    *
    * @param config
    *   Game-specific reconnection configuration
    * @param reason
    *   The failure reason from server
    */
