...
/** Manages WebSocket keepalive pings to prevent idle timeouts.
  *
  * Many proxies and load balancers (including Railway) have idle timeouts that close WebSocket connections after a
  * period of inactivity (typically 60-120 seconds). This module sends periodic ping messages to keep connections alive.
  *
  * Usage:
  * {{{
...
object WebSocketKeepAlive:
  ...
  /** Create a keepalive manager that sends a message via WebSocket.
    *
    * @param name
    *   Identifier for logging (e.g., game name)
    * @param getWebSocket
    *   Function to get the current WebSocket (if connected)
    * @param buildPingMessage
    *   Function to build the ping message string
    * @param intervalMs
    *   Ping interval in milliseconds (default: 20 seconds)
    */
  def forWebSocket(
      name: String,
