...
def parseRoute(pathname: String): GameRoute =
  pathname.split("/").filter(_.nonEmpty).toList match
    ...
    case "velor-idle" :: _            => GameRoute(AppRoute.VelorIdle, None)
...
def clientMain(pathnameOverride: Option[String]): Unit =
  ...
  gameRoute.route match
    ...
    case AppRoute.VelorIdle =>
      println("[client] Routing to Velor Idle...")
      safeInitialize(client.initializeVelorIdle())
