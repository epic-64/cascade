...
/**
 * Trait for entities that track connection state.
 * Used by player models in games to support reconnection.
 */
...
object PlayerConnectionOps:
   ...
   *
   * @param player The player connection state
   * @param gracePeriodMs How long after disconnect a player can rejoin
   * @return true if player can rejoin
   */
  ...
  /**
   * Check if a player's disconnect time has exceeded the grace period.
   * Useful for cleanup operations.
   *
   * @param player The player connection state
   * @param gracePeriodMs How long after disconnect before considered expired
  ...
  /**
   * Helper to mark a player as disconnected.
   * Returns the values to set on the player model.
   */
  def disconnectValues: (Boolean, Option[Long]) =
    (false, Some(System.currentTimeMillis()))

  /**
   * Helper to mark a player as reconnected.
   * Returns the values to set on the player model.
   */
